...
(function($){
	...
	document.writeln('<style>'+
						...
						'.set-time-'+class_num+' .set-time-'+class_num+'-content div{float:left;margin:2px;display:inline;}' +
	...
	$.fn.extend({
		setTime : function(options){
			...
			self.attr('readonly','true').bind('click', function(e){
				...
				//关闭
				$('body').on('click','#set-time-'+id_num+'-close', function(){
					clearData(thisHtmlObj,self)
				});
				$('body').on('mouseenter','.set-time-'+class_num+'', function(){
				...
				//选择时间
				$('body').on('click','#set-time-'+id_num+'-h td.use', function(){
				...
				$('body').on('click','#set-time-'+id_num+'-i td.use', function(){
				...
				$('body').on('click','#set-time-'+id_num+'-s td.use', function(){
