...
if(!class_exists('m_base_config')) require 'model/base/table/m_base_config.php';
/**
 * 基础配置业务
 * @name lib_config.php
 * @package cws
 ...
 * @since 2016-08-02
 */
class lib_config extends base_model{
    private $m_config;
    ...
    function __construct(){
        parent::__construct();
        $this->m_config = new m_base_config();
    }
	...
	public function getConfigForGet(){
		try {
			$reslut = $this->m_config -> findSql("SELECT * FROM base_config WHERE type != 'hidden' ORDER BY sort ASC");
			if($reslut){
	...
	public function findAllConfig($condition){
		try {
			$reslut = $this->m_config -> findAll($condition);
			if(true == $reslut){
				return common::errorArray(0, "查询成功",$reslut);
			}else{
				return common::errorArray(1, "查询为空",$reslut);
	...
	public function clearCache($module = 'all'){
		spAccess('c',  'configCache');//config缓存必然修改
	}
}