...
if(!class_exists('m_fen_cash_account')) require "model/fen/table/m_fen_cash_account.php";

/**
 * 提供用户账户服务
 * @name lib_cash_account.php
 * @package cws
 * @category model
 ...
 * @since 2016-08-04
 */
class lib_cash_account extends base_model{
    private $m_cash_account;
    public $tableConfigObj;//配置表对象
    
    function __construct(){
        parent::__construct();
        $this->m_cash_account = new m_fen_cash_account();
	 ...
	 * @return array
	 */
	function setDefaultAccount($id,$distributor_id){
		try{
			$sql = "UPDATE fen_cash_account SET is_default = 0 WHERE distributor_id = {$distributor_id}";
			$result = $this->m_cash_account->runSql($sql);
			$sql = "UPDATE fen_cash_account SET is_default =1 WHERE id = {$id}";
			$result = $this->m_cash_account->runSql($sql);
			if(true == $result){
				return common::errorArray(0, "修改成功", $result);
			}else{
	...
	function deleteAccountBatch($ids){
		try{
			$sql = "DELETE FROM fen_cash_account WHERE id in({$ids})";
			$result = $this->m_cash_account->runSql($sql);
			if(true == $result){
				return common::errorArray(0, "删除成功", $result);
			}else{
