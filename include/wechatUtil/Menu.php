...
class Menu{
	/*
	  Menu::setMenu($menuList);
            ...
            $menuList ＝ array(
                ...
                array('id'=>'3', 'pid'=>'1', 'name'=>'分类一子分类二', 'type'=>'view', 'code'=>'http://www.lanecn.com'),

                array('id'=>'4', 'pid'=>'0', 'name'=>'顶级分类二', 'type'=>'view', 'code'=>'http://www.php.net/'),

                array('id'=>'5', 'pid'=>'0', 'name'=>'顶级分类三', 'type'=>'view', 'code'=>'lane_wechat_menu_3'),
    ...
    public static function setMenu($menuList){
        ...
        foreach($menuList as $key=>$menu){
            ...
            if(isset($menu['sub_button'])){
                ...
                foreach($menu['sub_button'] as $k=>$son){
                    ...
                    }else if(@$son['type'] == 'media_id'){
            			$menuList[$key]['sub_button'][$k]['media_id'] = $son['code'];
           			}else if(@$son['type'] == 'miniprogram'){
           				$menuList[$key]['sub_button'][$k]['url'] = $son['code'];
           				$menuList[$key]['sub_button'][$k]['url'] = urlencode($menuList[$key]['sub_button'][$k]['url']);
           				$menuList[$key]['sub_button'][$k]['pagepath'] = urlencode($menuList[$key]['sub_button'][$k]['pagepath']);
           			}else{
                        $menuList[$key]['sub_button'][$k]['key'] = $son['code'];
        ...
        $data = urldecode($data);chromephp::info($data);
        //获取ACCESS_TOKEN
