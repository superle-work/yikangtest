    ...
    function bindEvent(){
        ...
        $("#content .withdraw a").click(function(){
        	...
        	if(model == 0){//线下打款
	        	...
	        	if(errorInfoData == 1){
	        		showDialog('#errorDialog','本月您已经操作过咯，再等等吧'," "," ",1500);
	        		return false;
	        	}else if(date.getDate() < limitTime){
	        		showDialog('#errorDialog','本月提现时间还没到，不能提现哦'," "," ",1500);
	        		return false;
	        	}
	            ...
	            if(checkedOption.length == 0){
	                showDialog('#errorDialog',"请选择收款账户！"," "," ",1500);
	                return false;
	            }
	            aid = checkedOption.attr("data-aid");//分销商账户表主键id
        	}else if(model == 2){//微信红包模式  微信红包限制2000元（包含）以内
        		if(money > 2000){
        			showDialog('#errorDialog',"抱歉，提现金额超额，微信零钱提现不能超过2000元"," "," ",1500);
        			return false;
        		}
        	}else{
        		if(money > 200){
        			showDialog('#errorDialog',"抱歉，提现金额超额，微信红包不能超过2000元"," "," ",1500);
        			return false;
        		}
        	}
            ...
            if( $("#content .cash-area .cash-value").focus().val() == '' || $("#content .cash-area .cash-value").focus().val() == 0){
	            showDialog('#errorDialog',"请填写金额"," "," ",1500);
            	return false;
            }
            //判断金额是否合法且在范围之内
            ...
            if(myfee < valuefee){
            	showDialog('#errorDialog',"填写的金额超过可提现的金额"," "," ",1500);
