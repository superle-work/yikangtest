$(function(){
    ...
    function bindEvent(){
        ...
        //置为默认地址
        $("#container .edit-address .check-box").click(function(){
        	var _this = $(this);
        	if(_this.children().hasClass('selected')){
        		return false;
        	}
			
			if($(".check-box .icon").hasClass('selected')){
				$(".check-box .icon").removeClass('selected').addClass('hidden');
			}
                ...
                $.ajax({
                    ...
                    success:function(json,statusText){
                        if(json.errorCode == 0){
                        	...
                        	showDialog("#successDialog",'操作成功','您的默认地址已成功修改','','2000',function(){
                        		_this.children().removeClass('hidden').addClass('selected');
                        		window.history.go(-1);
                        	});
