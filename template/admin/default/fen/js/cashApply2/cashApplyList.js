        ...
        $(".deletebatch").click(function(){
            ...
            if(ids == ""){
	            ...
	            myConfirmModal("确定要批量删除分销商提现申请吗？",function(){
		            $.ajax({
		                url:"./admin.php?c=fen_distributor&a=deleteCashApplyBatch",
		                type:"post",
		                data:{"ids":ids},
		                dataType:"json",
    ...
    function deleteOne(){
	     ...
	     if(isCheck != 0){
	    	myConfirmModal("确定要删除分销商提现申请吗？",function(){
	            $.ajax({
	                url:"./admin.php?c=fen_distributor&a=deleteCashApply",
	                type:"post",
	                data:{"id":id},
	                dataType:"json",
	                beforeSend:function(xhr){
...
//  自定义修改打款状态对话框，返回true或false,改变打款状态
    ...
    function editStatic(){
    	...
    	cashStaticModal("请选择打款状态",function(cash_static){
            $.ajax({
                url:"./admin.php?c=fen_distributor&a=editCashStaticApply",
                type:"post",
    ...
    function payCash(){
    	...
    	var id = $(this).attr("data_id");
        $.ajax({
            url:"./admin.php?c=fen_distributor&a=sendRedPacket",
            type:"post",
            data:{"id":id},
            dataType:"json",
            beforeSend:function(xhr){
...
//        var isCheck = $(this).attr("data");
        myCheckModal(noticeText,function(isCheck,info){
	        ...
	        $.ajax({
	            url:"./admin.php?c=fen_distributor&a=checkCashApply",
	            type:"post",
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:'./admin.php?c=fen_distributor&a=pagingCashApply',
            ...
            success:function(result){
                ...
                if(result.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="my-icheckbox select-all"></th><th class="th2">序号</th><th class="th3">分销商名称</th><th class="th4">佣金总量</th><th class="th5">提现金额</th><th class="th8">申请时间</th><th class="th9">审核时间</th><th class="th10">审核状态</th><th class="th11">提现状态</th><th class="th12">审核人</th><th class="th13">操作</th></tr>';
...
//                        var static_display = (obj.cash_static != 1 && obj.cash_static != 2) ? 'none' : 'inline';//是否显示打款状态按钮
                        ...
                        if(model == 0){//1：零钱模式  0：线下打款
                        	viewAccount = '<a class="btn btn-xs btn-primary view" href="./admin.php?c=fen_distributor&a=cashAccountDetail&account_id='+account_id+'">查看账户</a>';
                        }
