        ...
        $(".deletebatch").click(function(){
            ...
            if(ids == ""){
	            ...
	            myConfirmModal("确定要批量删除分销商提现申请吗？",function(){
		            $.ajax({
		                url:"./admin.php?c=base_user&a=deleteCashApplyBatch",
		                type:"post",
		                data:{"ids":ids},
		                dataType:"json",
    ...
    function deleteOne(){
	     ...
	     if(isCheck != 0){
	    	myConfirmModal("确定要删除分销商提现申请吗？",function(){
	            $.ajax({
	                url:"./admin.php?c=base_user&a=deleteCashApply",
	                type:"post",
	                data:{"id":id},
	                dataType:"json",
	                beforeSend:function(xhr){
...
//  自定义修改打款状态对话框，返回true或false,改变打款状态
    function cashStaticModal(alertInfo,callback){//参数，当前操作提示文本
    	...
    	//打款中
        $("#cashStaticModa cash").one('click',function(){
    ...
    function editStatic(){
    	...
    	cashStaticModal("请选择打款状态",function(cash_static){
            $.ajax({
                url:"./admin.php?c=base_user&a=editCashStaticApply",
                type:"post",
    ...
    function payCash(){
    	...
    	var id = $(this).attr("data_id");
        $.ajax({
            url:"./admin.php?c=base_user&a=sendRedPacket",
            type:"post",
            data:{"id":id},
            dataType:"json",
            beforeSend:function(xhr){
  ...
  //自定义审核确认对话框，返回true或false,通过或不通过审核操作
    function myCheckModal(alertInfo,callback){//参数，当前操作提示文本
         ...
         //审核不通过对话框--不通过操作
         $("#myCheckModal .btn-error").one('click',function(){//多次点击
    ...
    function check(){
    	...
    	if($(this).attr("data-model") == 1){//审核通过自动发零钱
...
//        var isCheck = $(this).attr("data");
        myCheckModal(noticeText,function(isCheck,info){
	        ...
	        $.ajax({
	            url:"./admin.php?c=base_user&a=checkCashApply",
	            type:"post",
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:'./admin.php?c=base_user&a=pagingCashApply',
            ...
            success:function(result){
                ...
                if(result.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="my-icheckbox select-all"></th><th class="th2">序号</th><th class="th3">用户昵称</th><th class="th4">余额总量</th><th class="th5">提现金额</th><th class="th8">申请时间</th><th class="th9">审核时间</th><th class="th10">审核状态</th><th class="th11">提现状态</th><th class="th12">审核人</th><th class="th13">操作</th></tr>';
...
//                        var static_display = (obj.cash_static != 1 && obj.cash_static != 2) ? 'none' : 'inline';//是否显示打款状态按钮
                        ...
                        if(model == 0){//1：零钱模式  0：线下打款
                        	viewAccount = '<a class="btn btn-xs btn-primary view" href="./admin.php?c=base_user&a=cashAccountDetail&account_id='+account_id+'">查看账户</a>';
                        }
                        //is_check 0未审核1审核通过2审核不通过
                        //state 打款状态0未打款1即将打款2打款中3打款成功（4打款失败，再次打款，零钱状态下）
                        if(is_check == 0 ){//未审核
                        	operateText = '<a class="btn btn-xs btn-danger check" href="javascript:;" data-model="'+model+'" data_id="'+id+'">审核</a>';
                        	...
                        	operateText = '<a class="btn btn-xs btn-danger pay-cash" href="javascript:;"  data_id="'+id+'" >重发零钱</a><a href="javascript:;" class="btn btn-default btn-xs delete" data_id="'+id+'">删除</a>';
                        	select_able = ' class="my-icheckbox select-single" ';
                        }else{
                        	if(model == 0){//线下打款
                        		operateText = '<a class="btn btn-xs btn-danger cash-static" href="javascript:;"  data_id="'+id+'"  cash_static="'+state+'">打款状态</a>';	
                        	}
//                        	select_able = ' class="my-icheckbox" ';
                        }
	                	html+='<tr>'
