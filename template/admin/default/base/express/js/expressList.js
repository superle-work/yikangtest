...
$(function(){
    ...
    function bindEvent(){
        ...
        $(".content .right-section .delete-batch").click(function(){
            ...
            if(ids == ""){
	            ...
	            myConfirmModal("确定要批量删除这些快递公司吗？",function(){
		            $.ajax({
		                url:"./admin.php?c=base_express&a=deleteExpress",
		                type:"post",
		                data:{"ids":ids},
		                dataType:"json",
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:'./admin.php?c=base_express&a=pagingExpress',
            ...
            success:function(json){
                ...
                if(json.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="select-all my-icheckbox"></th><th class="th2">序号</th><th class="th3">快递名称</th><th class="th4">快递编码</th><th class="th5">状态</th><th class="th6">排序</th><th class="th7">操作</th></tr>';
                    ...
                    for(var i = 0; i < myList.length;i++){
                        ...
                        var operate = '';
                        if(is_use == 1){
                        	useText = "<span style='color:green'>已启用</span>";
                        	operate = '<a href="javascript:;" data-id="'+id+'" class="btn btn-danger btn-xs down-express">停用</a>';
                        }else{
                        	useText = "<span style='color:#EE0000'>已停用</span>";
                        	operate = '<a href="javascript:;" data-id="'+id+'" class="btn btn-success btn-xs up-express">启用</a>';
                        }
                        ...
                        html+='<tr>'
                        		+'<td><input type="checkbox" class="select-single my-icheckbox" value="'+id+'" '+checked+'></td>'
                                 ...
                                 +'<td>'+sort+'</td>'
                                 +'<td>'
                                 	+'<a href="./admin.php?c=base_express&a=editExpress&id='+id+'" class="btn btn-primary btn-xs editor">编辑</a>'
                                 	+operate
                    ...
                    //停用
                    $("#list-table .down-express").click(function(){
                    	id = $(this).attr('data-id');
                    	setExpressState(id,0);
                    });
                    //启用
                    $("#list-table .up-express").click(function(){
                    	id = $(this).attr('data-id');
                    	setExpressState(id,1);
                    });
                }else{
	...
	function deleteService() {
        var id = $(this).attr("id");
        myConfirmModal("确定要删除该快递公司吗？",function(){
            $.ajax(
                {
                    type:"post",
                    url:"./admin.php?c=base_express&a=deleteExpress",
	 ...
	 * @param int is_use
	 */
	function setExpressState(id, is_use){
		$.ajax(
	            {
	                type:"post",
	                url:"./admin.php?c=base_express&a=setExpressState",
