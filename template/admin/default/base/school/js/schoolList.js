$(function(){
    ...
    function bindEvent(){
        ...
        $(".content .right-section .delete-batch").click(function(){
            ...
            if(ids == ""){
	            ...
	            myConfirmModal("确定要批量删除校区吗？",function(){
		            $.ajax({
		                url:"./admin.php?c=base_school&a=deleteSchool",
		                type:"post",
		                data:{"ids":ids},
		                dataType:"json",
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:'./admin.php?c=base_school&a=pagingSchool',
            ...
            success:function(json){
                ...
                if(json.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="select-all my-icheckbox"></th><th class="th3">序号</th><th class="th2">省份</th><th class="th2">城市</th><th class="th2">地区</th><th class="th2 hidden">学校</th><th class="th2">邮费</th><th class="th3">排序</th><th class="th4">操作</th></tr>';
                    ...
                    for(var i = 0; i < myList.length;i++){
                        ...
                        var fee = obj.fee||'0.00';
                        var id = obj.id;
                        var sort = obj.sort_num;
                        ...
                        html+='<tr>'
                        		+'<td><input type="checkbox" class="select-single my-icheckbox" value="'+id+'" '+checked+'></td>'
                                 ...
                                 +'<td>'+ sort +'</td>'
                                 +'<td>'
                                 	+'<a href="./admin.php?c=base_school&a=editSchool&id='+id+'" class="btn btn-primary btn-xs editor">编辑</a>'
	...
	function deleteService() {
        var id = $(this).attr("id");
        myConfirmModal("确定要删除校区吗？",function(){
            $.ajax(
                {
                    type:"post",
                    url:"./admin.php?c=base_school&a=deleteSchool",
