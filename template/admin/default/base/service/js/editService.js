$(function(){
     ...
     */
    function init(){
    	$(".content .inner-section .head_img_url").each(function(){
    		if($(this).val() == $(".content .inner-section .prev_head_img_url").val()){
    ...
    function bindEvent(){
    	...
    	$('#myForm #save').click(function() {
    		...
    		var weichat_number = $('#myForm #weichat_number').val();
    		var prevUrl = $("#myForm #prevUrl").val() || '';
    		var phone = $.trim($("#myForm #phone_number").val());
    		var qq = $.trim($("#myForm #qq_number").val());
        	if((type == 1 && qq == '') || (type ==2 &&  weichat_number == '' && prevUrl == '') || (type == 3 && phone == '')){
        		var text = "";
        		if(type == 1){
        			text = "请填写客服QQ！";
        		}else if(type ==2){
    ...
    var serviceFormOptions={
        url:'./admin.php?c=base_service&a=updateService',
