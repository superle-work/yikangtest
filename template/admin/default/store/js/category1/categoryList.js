	            ...
	            $.ajax({
	                ...
	                success:function(json){
	                    if(json.errorCode == 0){
	                        ...
	                        $.ajax({
	                            type:'post',
	                            url:'./admin.php?c=store_category&a=deleteCategory',
	                            data:selectInfo,
	                            dataType:'json',
	                            success:function(json){
	                                if(json.errorCode == 0){
                ...
                if(json.errorCode == 0){
                    ...
                    for(var i=0,l=data.length;i<l;i++){
                        ...
                        html += '<tr class="cat-level-'+selectInfo.rank+' cat-stored" id='+data[i].id+' rank="1" fir="'+(cate.fir)+'" sec="'+cate.sec+'" thr="'+cate.thr+'" fou="'+cate.fou+'">'
                            ...
                            +'<td class="td-0"><span class="arrow arrow-left hidden" data-rank="'+selectInfo.rank+'"></span></td>'
                            ...
                            +'<td class="td-3"><a href="./admin.php?c=store_category&a=cateGoodsList&id='+data[i].id+'" >商品集</a>'+templateHtml+'<a class="cat-statusOper" data-rank="1" status="'+data[i].is_use+'">'+statusOper+'</a><a class="cat-delete"  data-rank="1">删除</a><a class="single-cat-save">保存</a></td></tr>';
