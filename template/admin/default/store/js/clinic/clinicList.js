$(function(){
    ...
    function bindEvent(){
        ...
        $(".content .right-section .delete-batch").click(function(){
            ...
            if(ids == ""){
	            ...
	            myConfirmModal("确定要批量删除诊所吗？",function(){
		            $.ajax({
		                url:"./admin.php?c=store_clinic&a=batchDelete",
		                type:"post",
		                data:{"ids":ids},
		                dataType:"json",
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:'./admin.php?c=store_clinic&a=pagingClinic',
            ...
            success:function(json){
                ...
                if(json.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="select-all my-icheckbox"></th><th class="th1">序号</th><th class="th3">缩略图</th><th class="th4">诊所名称</th><th class="th11">推荐ID</th><th class="th11">电话</th><th class="th3">可提佣金</th><th class="th3">累计佣金</th><th class="th3">诊所采样费用</th><th class="th3">排序系数</th><th class="th8">地址</th><th class="th8">创建日期</th><th class="th2">操作</th></tr>';
                    var colspan = $(html).find("th").length;
                    for(var i = 0; i < myList.length;i++){
                        var obj = myList[i];
                        var num = (pageIndex-1)*pageSize + i+1;
                        var name = obj.name;
                        var clinicId = obj.clinicId;
                        var createtime = obj.add_time;
                        var phone = obj.phone;
                        var money = obj.money;
                        var total_fee = obj.total_fee;
                        var clinic_ratio = obj.clinic_ratio;
                        ...
                        html+='<tr>'
                                +'<td><input type="checkbox" class="select-single my-icheckbox" value="'+cid+'" '+checked+'></td>'
                                 ...
                                 +'<td><a href="./admin.php?c=store_clinic&a=clinicDetail&id='+cid+'" class="limit-text">'+ name +'</a></td>'
                                 +'<td>'+clinicId+'</td>'
                                 +'<td>'+phone+'</td>'
                                 +'<td>'+money+'</td>'
                                 +'<td>'+total_fee+'</td>'
                                 ...
                                 +'<td>'+createtime+'</td>'
                                 +'<td>'
                                    +'<a style="margin-bottom:5px;" href="./admin.php?c=store_clinic&a=showUser&id='+cid+'&uid='+uid+'" class="btn btn-success btn-xs">查看工作人员</a>'
                                    +'<a style="margin-bottom:5px;" href="./admin.php?c=store_clinic&a=cancelOrder&id='+cid+'" class="btn btn-info btn-xs">核销记录</a>'
                                    +'<a style="margin-bottom:5px;" href="javascript:;" data-cid="'+cid+'" class="btn btn-success btn-xs pay">结算</a>'
                                    +'<a href="./admin.php?c=store_clinic&a=editClinic&id='+cid+'" class="btn btn-primary btn-xs editor">编辑</a>'
	...
	function deleteClinic() {
        ...
        myConfirmModal("确定要删除诊所吗？",function(){
            $.ajax(
                {
                    type:"post",
                    url:"./admin.php?c=store_clinic&a=deleteClinic",
	...
	function payClinic(){
		...
		$("#myConfirmModal .modal-footer .btn-confirm").click(function(){
			...
			$.ajax({
				type:'post',
				url:'admin.php?c=store_clinic&a=payMoney',
				data:{
					'id':cid,
				},
				dataType:'json',
