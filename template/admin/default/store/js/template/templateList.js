$(function(){
      ...
      //批量删除
        $(".content .right-section .delete-batch").click(function(){
            ...
            if(ids == ""){
	            ...
	            myConfirmModal("确定要批量删除模板吗？",function(){
		            $.ajax({
		                url:"./admin.php?c=store_template&a=batchDeleteTemplate",
		                type:"post",
		                data:{"ids":ids},
		                dataType:"json",
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:'./admin.php?c=store_template&a=pagingTemplate',
            ...
            success:function(result){
                ...
                if(result.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="select-all my-icheckbox"></th><th>序号</th><th>模板名称</th><th>操作</th></tr>';
                    var colspan = $(html).find("th").length;
                    for(var i = 0; i < myList.length;i++){
                        var obj = myList[i];
                        var num = (pageIndex-1)*pageSize + i+1;
                        var name = obj.name;
                        var cname = obj.cname?obj.cname:"--";//分类名称
                        var id = obj.id;
                        var cid = obj.cid;
                        ...
                        html+='<tr>'
                        	+'<td><input type="checkbox" class="select-single my-icheckbox" value="'+id+'" '+checked+'></td>'
                                +'<td>'+num+'</td>'
                                +'<td><a href="./admin.php?c=store_template&a=editTemplate&id='+id+'"><span>'+ name +'</span></a></td>'
                                +'<td>'
                                    +'<a class="btn btn-xs btn-primary" href="./admin.php?c=store_template&a=editTemplate&id='+id+'">编辑</a>'
                                    +(cid > 0?'<a href="javascript:;" class="btn btn-xs btn-warning deleteBind" tid="'+id+'" cid="'+cid+'">解绑</a>':"")
                                    +'<a href="javascript:;" class="btn btn-xs btn-default delete" tid = "'+id+'">删除</a>'
                                +'</td>'
                            +'</tr>';
                    ...
                    if(myList.length == 0){
                        ...
                        //解绑
                        $("#list-table .deleteBind").click(deleteBind);

                        //删除
                        $("#list-table .delete").click(deleteTemplate);
                    }
    ...
    function deleteBind(){
        ...
        myConfirmModal("确定解绑吗？",function(){
            $.ajax(
                {
                    type:"post",
                    url:"./admin.php?c=store_template&a=deleteBind",
    ...
    function deleteTemplate() {
        ...
        myConfirmModal("确定删除当前模板吗？",function(){
            $.ajax({
                type:"post",
                url:"./admin.php?c=store_template&a=deleteTemplate",
                data:{"id":id},
                dataType:"json",
                beforeSend:function(xhr){
                    //显示“加载中。。。”
