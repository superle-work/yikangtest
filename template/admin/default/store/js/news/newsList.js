$(function(){
        ...
        $(".content .right-section .delete-batch").click(function(){
            ...
            if(ids == ""){
	            ...
	            myConfirmModal("确定要批量删除模板吗？",function(){
		            $.ajax({
		                url:"./admin.php?c=store_news&a=batchDeleteNews",
		                type:"post",
		                data:{"ids":ids},
		                dataType:"json",
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:'./admin.php?c=store_news&a=pagingNews',
            ...
            success:function(result){
                ...
                if(result.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="select-all my-icheckbox"></th><th>序号</th><th>消息标题</th><th>消息链接</th><th>操作</th></tr>';
                    ...
                    for(var i = 0; i < myList.length;i++){
                        ...
                        html+='<tr>'
                        	+'<td><input type="checkbox" class="select-single my-icheckbox" value="'+id+'" '+checked+'></td>'
                                ...
                                + '<td>'+ link +'</td>'
                                +'<td>'
                                    +'<a class="btn btn-xs btn-primary" href="./admin.php?c=store_news&a=editNews&id='+id+'">编辑</a>'
                    ...
                    if(myList.length == 0){
                        ...
                        //删除
                        $("#list-table .delete").click(deleteNews);
                    }
    ...
    function deleteNews() {
        ...
        myConfirmModal("确定删除该条消息么？",function(){
            $.ajax({
                type:"post",
                url:"./admin.php?c=store_news&a=deleteNews",
                data:{"id":id},
                dataType:"json",
                beforeSend:function(xhr){
                    //显示“加载中。。。”
