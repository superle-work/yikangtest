        ...
        $(".delete-batch").click(function(){
            ...
            if(ids == ""){
	            ...
	            myConfirmModal("确定要批量删除评价信息吗？",function(){
		            $.ajax({
		                url:"./admin.php?c=store_comment&a=deleteCommentBatch",
		                type:"post",
		                data:{"ids":ids},
		                dataType:"json",
        ...
        $(".replybatch").click(function(){
            ...
            if(ids == ""){
	            ...
	            myReplyModal("请填写回复内容",function(info){
		            $.ajax({
		                url:"./admin.php?c=store_comment&a=replyCommentBatch",
		                type:"post",
    ...
    function deleteOne(){
	     ...
	     if(isCheck != 0){
	    	myConfirmModal("确定要删除该条评价信息吗？",function(){
	            $.ajax({
	                url:"./admin.php?c=store_comment&a=deleteComment",
	                type:"post",
	                data:{"id":id},
	                dataType:"json",
	                beforeSend:function(xhr){
...
//        var isCheck = $(this).attr("data");
        myReplyModal("请填写回复内容",function(info){
	        $.ajax({
	            url:"./admin.php?c=store_comment&a=replyComment",
	            type:"post",
...
//	                      window.location.reload();
    ...
    function render(async,pageIndex,pageSize){
        ...
        $.ajax({
            async:async,
            type:'post',
            url:'./admin.php?c=store_comment&a=pagingComment',
            ...
            success:function(result){
                ...
                if(result.errorCode == 0){
                    ...
                    html+='<tr><th class="th1"><input type="checkbox" class="select-all my-icheckbox"></th><th class="th2">序号</th><th class="th3">昵称</th><th class="th8">订单编号</th><th class="th4">商品名</th><th class="th5">评分</th><th class="th6">等级</th><th class="th7">评价内容</th><th class="th8">评价时间</th><th class="th9">图片</th><th class="th9">匿名</th><th class="th10">是否回复</th><th class="th11">操作</th></tr>';
                    ...
                    for(var i = 0; i < commentList.length;i++){
                       ...
                       if(obj.is_anonym == 1){
                    	...
                    	html+='<tr>'
                    			+'<td><input type="checkbox" value="'+id+'"  data_id="'+id+'"'+disableText+'></td>'
                    			+'<td>'+number+'</td>'
                    			+'<td><a href="./admin.php?c=base_user&amp;a=userDetail&amp;id='+user_id+'" class="limitName" title="查看用户详情">'+nick_name+'</a></td>'
                    			+'<td><a href="./admin.php?c=store_order&a=orderDetailList&oid='+oid+'" title="查看订单详情">'+order_num+'</a></td>'
                    			+'<td><a href="./admin.php?c=store_goods&amp;a=goodsDetail&amp;id='+gid+'" class="limitName" title="查看商品详情">'+goods_name+'</a></td>'
                    			+'<td>'+score+'</td>'
                    			+'<td>'+level+'</td>'
    ...
    function commentOperation(id,is_reply){
        ...
        {
            case '1'://已回复
                html +='<a class="btn btn-xs btn-primary view" href="./admin.php?c=store_comment&amp;a=commentDetail&amp;id='+id+'">查看</a>';
