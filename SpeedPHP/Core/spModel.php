...
class spModel {
	...
	/**
	 * 批量插入
	 * @param string $tableName cg_student
	 * @param string $fieldString name,sex
	 * @param array $rowList array(array('张三','26'),array('李四','30'))
	 * @return boolean
	 */
	public function createBatch($tableName,$fieldString,$rowList){
		if(!$tableName && !$fieldString && !$rowList && !is_array($rowList) && count($rowList) <= 0){
			return false;
		}
		$sql = "INSERT INTO {$tableName}({$fieldString}) VALUES ";
		$valueString = "";
	 ...
	 * @return array
	 */
	public function paging($page,$conditionList,$sort = '',$orList = null)
	{
		...
		if(count($conditionList) > 0){
			...
			$sql = "SELECT * FROM {$this->tbl_name} WHERE{$whereString} ";
			if($orString){
				$sql = "{$sql}AND {$orString} ";
			}
		}else{
			if($orString){
				$sql = "SELECT * FROM {$this->tbl_name} WHERE {$orString}";
			}else{
		...
		try {
			$result['dataList'] = $this->findSql($sqlLimit);
	 ...
	 * @return array
	 */
	public function pagingJoin($table,$page,$conditionList,$sort = '')
	{
		$page['pageIndex'] ? $pageIndex = $page['pageIndex'] : $pageIndex = 1;
		$page['pageSize'] ? $pageSize = $page['pageSize'] : $pageSize = 10;
			...
			foreach ($conditionList as $condition){
				...
				if($firList[0] == 'lf'){
					...
					if('like' == $condition['operator']){
						...
						$value = $this->escape($condition['value']);
						$leftWhereString .= " {$condition['field']} {$condition['operator']} {$value} AND";
					}
				}elseif($firList[0] == 'rg'){
					unset($firList[0]);
					$condition['field'] = implode('_',$firList);
					if('like' == $condition['operator']){
						...
						$value = $this->escape($condition['value']);
						$rightWhereString .= " a.{$condition['field']} {$condition['operator']} {$value} AND";
					}
				}
			}
	 ...
	 * @return string
	 */
	public function pagingWhere($conditionList){
	    //和连接条件
	    $whereString = "";
	    if(count($conditionList) > 0){
	        foreach ($conditionList as $condition){
	            if('like' == $condition['operator'] || 'not like' == $condition['operator']){
	                $whereString .= " {$condition['field']}  {$condition['operator']} '%{$condition['value']}%' AND";
	            }else if('in' == $condition['operator']){
	                $whereString .= " {$condition['field']} in ({$condition['value']}) AND";
	            }else{
	                $value = $this->escape($condition['value']);
	 ...
	 * @return array $result
	 */
	public function pagingCommon($page,$sql,$sort = '')
	{
	    $page['pageIndex'] ? $pageIndex = $page['pageIndex'] : $pageIndex = 1;
	    $page['pageSize'] ? $pageSize = $page['pageSize'] : $pageSize = 10;
	    //排序
