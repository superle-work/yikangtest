...
define('JS_PATH',"../../js");
$db = new db();
$configInfo = $db->find("SELECT * FROM base_config WHERE item_key = 'drive_theme'");
$curTemplate = $configInfo['item_value'];
$smarty->assign('theme', $curTemplate);
$authInfo =  WeChatOAuth::getAccessTokenAndOpenId($_GET['code']);
//获取订单对象
$orderInfo = $db->find("SELECT * FROM drive_order WHERE id = {$_GET['state']}");
//获取用户对象
$userInfo = $db->find("SELECT * FROM base_user WHERE account = {$orderInfo['account']}");
//微信支付标题
$wechatPayTitleSql = "SELECT * FROM drive_config WHERE item_key = 'wechat_pay_title'";
...
$orderArgsObj->SetNotify_url(WxPayConfig::DRIVE_NOTIFY_URL);//异步通知url
...
try{
	...
	header("location: {$configInfo['wechat_url']}/index.php?c=drive&a=index");
}
$smarty->assign('jsApiParameters', $jsApiParameters);
$smarty->assign('orderInfo', $orderInfo);
//商城支付类型
$wechatPayTypeSql = "SELECT * FROM drive_config WHERE item_key = 'pay_type'";
$wechatPayType = $db->find($wechatPayTypeSql);
$smarty->assign('payType', $wechatPayType['item_value']);
$smarty->display($_SERVER['DOCUMENT_ROOT']."/template/front/drive/{$curTemplate}/page/order/paymentOrder.html");
