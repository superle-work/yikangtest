...
define('JS_PATH',"../../js");
$db = new db();
$configInfo = $db->find("SELECT * FROM base_config WHERE item_key = 'rob_theme'");
$curTemplate = $configInfo['item_value'];
$smarty->assign('theme', $curTemplate);
$authInfo =  WeChatOAuth::getAccessTokenAndOpenId($_GET['code']);
//获取订单对象
$orderInfo = $db->find("SELECT * FROM rob_order WHERE id = {$_GET['state']}");
//获取用户对象
$userInfo = $db->find("SELECT * FROM base_user WHERE id = {$orderInfo['user_id']}");
//解析商品属性
$orderInfo['goods_info'] = json_decode($orderInfo['goods_info']);
$url = explode('./', $orderInfo['goods_info']->thumb );
$orderInfo['goods_info']->thumb =  WECHAT_URL . $url[1];
$smarty->assign('addrees_text', $orderInfo['addrees_text']);
//微信支付标题
$wechatPayTitleSql = "SELECT * FROM rob_config WHERE item_key = 'wechat_pay_title'";
...
try{
	...
	header("location: {$configInfo['wechat_url']}/index.php?c=rob&a=index");
}
$smarty->assign('jsApiParameters', $jsApiParameters);
$smarty->assign('orderInfo', $orderInfo);
//商城支付类型
$wechatPayTypeSql = "SELECT * FROM rob_config WHERE item_key = 'pay_type'";
$wechatPayType = $db->find($wechatPayTypeSql);
$smarty->assign('payType', $wechatPayType['item_value']);
$smarty->display($_SERVER['DOCUMENT_ROOT']."/template/front/rob/{$curTemplate}/page/order/paymentOrder.html");
