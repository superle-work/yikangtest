...
define('JS_PATH',"../../js");
$db = new db();
$configInfo = $db->find("SELECT * FROM base_config WHERE item_key = 'photography_theme'");
$curTemplate = $configInfo['item_value'];
$smarty->assign('theme', $curTemplate);
$authInfo =  WeChatOAuth::getAccessTokenAndOpenId($_GET['code']);
//获取订单对象
$orderInfo = $db->find("SELECT * FROM photography_order WHERE id = {$_GET['state']}");
//解析商品属性
...
//微信支付标题
$wechatPayTitleSql = "SELECT * FROM photography_order WHERE item_key = 'wechat_pay_title'";
$wechatPayTitle = $db->find($wechatPayTitleSql);
//订单产品信息
$orderInfo['goods_list'] = json_decode($orderInfo['goods_list']);
$smarty->assign('goods_list', $orderInfo['goods_list']);
//微信支付接口
...
$orderArgsObj->SetNotify_url(WxPayConfig::PHOTOGRAPHY_NOTIFY_URL);//异步通知url
...
try{
	...
	header("location: {$configInfo['wechat_url']}/index.php?c=photography&a=userCenter");
}
$smarty->assign('jsApiParameters', $jsApiParameters);
$smarty->assign('orderInfo', $orderInfo);
//商城支付类型
$wechatPayTypeSql = "SELECT * FROM photography_config WHERE item_key = 'pay_type'";
$wechatPayType = $db->find($wechatPayTypeSql);
$smarty->assign('payType', $wechatPayType['item_value']);
$smarty->display($_SERVER['DOCUMENT_ROOT']."/template/front/photography/{$curTemplate}/page/order/confirmPaymentOrder.html");
