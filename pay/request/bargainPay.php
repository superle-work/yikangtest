...
//获取订单对象
$orderInfo = $db->find("SELECT * FROM bargain_order WHERE id = {$_GET['state']}");
//获取用户对象
...
//微信支付标题
$wechatPayTitleSql = "SELECT * FROM bargain_config WHERE item_key = 'wechat_pay_title'";
...
$orderArgsObj->SetNotify_url(WxPayConfig::BARGAIN_NOTIFY_URL);//异步通知url
...
try{
	...
	header("location: {$configInfo['wechat_url']}/index.php?c=bargain&a=index");
}
$smarty->assign('jsApiParameters', $jsApiParameters);

//是否开启了余额支付
$isbalanceSql = "SELECT * FROM bargain_config WHERE item_key = 'is_balance'";
$isbalance = $db->find($isbalanceSql);
$smarty->assign('isbalance', $isbalance['item_value']);
// //用户优惠券信息
...
// }
// //会员卡信息
// $wechatCardSql = "SELECT * FROM bargain_config WHERE item_key = 'mall_card'";
...
$smarty->display($_SERVER['DOCUMENT_ROOT']."/template/front/bargain/{$curTemplate}/page/order/paymentOrder.html");
