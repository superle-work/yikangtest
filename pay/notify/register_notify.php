...
if( $postObj->return_code == "SUCCESS"){
		if($postObj->result_code == "SUCCESS"){ 			
			...
			$orderInfo = $db->find("SELECT * FROM register_order WHERE order_num =  '{$orderNum}'");
			if($orderInfo['transaction_id']){//订单已处理直接结束
				echo 'success';
				exit;
			}
			//更新订单信息
			$db->update("register_order", $orderInfoData, "order_num = '{$orderNum}'");
			//更新商品信息
			$goodsList = json_decode($orderInfo['goods_list']);
			foreach ($goodsList as $goods){
				if($goods->gpid){//多规格
					//获取商品属性信息
					$goodsProperty = $db->find("SELECT * FROM register_goods_property WHERE id =  {$goods->gpid}");
					//获取新库存
					...
					//修改库存
					$db->update("register_goods_property", $goodsPropertyInfo, "id = {$goods->gpid}");
				}else{//统一规格
					//获取最新商品信息
					$oriGoodsInfo = $db->find("SELECT * FROM register_goods WHERE id =  {$goods->gid}");
					$goodsInfo = array(
								'inventory' => $oriGoodsInfo['inventory'] -  $goods->count//减库存
							);
					$db->update("register_goods", $goodsInfo, "id = {$goods->gid}");
				}
			}
// 			//更新用户信息   移动至交易完成部分
...
// 					);
// 			//查看是否开启积分
// 			$openPoint = $db->find("SELECT * FROM register_config WHERE name = 'mall_point'");
// 			if($openPoint['value'] == '1'){
// 			    $exchange = $db->find("SELECT * FROM register_config WHERE name = 'point_exchange_rate'");
...
// 			$db->update("base_user", $userInfoData, "account = '{$orderInfo['account']}'");
			//订单支付成功通知
			$keyInfo = $db->find("SELECT * FROM register_config WHERE item_key =  'order_notify_key'");//获取密钥
			$url = "http://{$_SERVER['HTTP_HOST']}/index.php?c=server&a=orderNotify&key={$keyInfo['item_value']}&oid={$orderInfo['id']}&module=register";
