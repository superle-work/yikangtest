...
if( $postObj->return_code == "SUCCESS"){
		if($postObj->result_code == "SUCCESS"){ 			
			...
			$orderInfo = $db->find("SELECT * FROM hotel_order WHERE order_num =  '{$orderNum}'");
			if($orderInfo['transaction_id']){//订单已处理直接结束
				echo 'success';
				exit;
			}
			//更新订单信息
			$db->update("hotel_order", $orderInfoData, "order_num = '{$orderNum}'");
            
			//订单支付成功通知
			$keyInfo = $db->find("SELECT * FROM hotel_config WHERE item_key =  'order_notify_key'");//获取密钥
			$url = "http://{$_SERVER['HTTP_HOST']}/index.php?c=server&a=orderNotify&key={$keyInfo['item_value']}&oid={$orderInfo['id']}&module=hotel";
