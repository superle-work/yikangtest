...
if( $postObj->return_code == "SUCCESS"){
		if($postObj->result_code == "SUCCESS"){ 			
			...
			$orderInfo = $db->find("SELECT * FROM dining_order WHERE order_num =  '{$orderNum}'");
			if($orderInfo['transaction_id']){//订单已处理直接结束
				echo 'success';
				exit;
			}
			//更新订单信息
			$db->update("dining_order", $orderInfoData, "order_num = '{$orderNum}'");
			//更新商品信息
			$goodsList = json_decode($orderInfo['goods_list']);
			foreach ($goodsList as $goods){
				//获取最新商品信息
				$oriGoodsInfo = $db->find("SELECT * FROM dining_goods WHERE id =  {$goods->gid}");
				$goodsInfo = array(
							'inventory' => $oriGoodsInfo['inventory'] -  $goods->count//减库存
						);
				$db->update("dining_goods", $goodsInfo, "id = {$goods->gid}");
			}
			//订单支付成功通知
			$keyInfo = $db->find("SELECT * FROM dining_config WHERE item_key =  'order_notify_key'");//获取密钥
			$url = "http://{$_SERVER['HTTP_HOST']}/index.php?c=server&a=orderNotify&key={$keyInfo['item_value']}&oid={$orderInfo['id']}&module=dining";
